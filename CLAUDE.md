...
- REST mutations broadcast **after** `response.json()` via `broadcastBoardUpdate(request, boardId, reason)` from `src/sockets/board.broadcast.ts`. It re-fetches a board snapshot and emits `BE_BOARD_UPDATED` to the board room. Sends are serialised per board through the `inFlight` map so a slow snapshot query for an earlier update cannot land after a newer one — the client replaces the whole board on every payload, so an out-of-order emit rolls it backwards and it stays there.
