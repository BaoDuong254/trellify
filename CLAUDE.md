...
- **Auth is injected by the `authenticate` hook into `defaultBrowserContext()`.** Lighthouse runs in a child process on a tab in the browser's default context, while puppeteer-cluster gives every job its own throwaway incognito context. The `cookies`/`localStorage` options therefore never reach the page being measured, and every protected route renders `/login`. `disableStorageReset` is on for the authenticated run so Lighthouse does not wipe `persist:root` before loading the page. The hook first trades `UNLIGHTHOUSE_REFRESH_TOKEN` for a fresh access token via `GET /api/v1/users/refresh_token` (refresh tokens are not rotated, so it works for its whole 14 days) and throws if that fails, so only the refresh token and `persist:root` live in `.env.unlighthouse`.
...
Internal packages skip the catalog and use `"workspace:*"`. `pnpm-workspace.yaml` sets `saveExact` and `engineStrict`. The repo has no `.npmrc`: since pnpm 11 it is read for registry and auth only, so a setting added there is silently ignored — put pnpm settings in `pnpm-workspace.yaml`. `engineStrict` checks dependencies' `engines` against `nodeVersion`; it does not stop a workspace running on the wrong Node. `devEngines.runtime` in the root `package.json` is what pins Node: pnpm downloads 24.21.0 and runs every script with it, whatever `node -v` says. Keep `engines.node` in every workspace `package.json` anyway: `eslint-plugin-n` reads it from the nearest `package.json` to decide which Node APIs are allowed, and without it falls back to `>=16.0.0` and fails lint on anything newer (`import.meta.dirname` in `e2e/support/environment.ts` is the first to break). A pre-commit hook (`scripts/check-lockfile.sh`) verifies the lockfile is in sync.
...
- **Invalidation has exactly one site**: `broadcastBoardUpdate` drops `c:v2:board:<id>` before the `getIo()` guard, which covers every board/column/card/invitation mutation. Add a mutation that does not broadcast and it will serve stale reads until the TTL. `PUT /v1/users/update` is the known one - `displayName`/`avatar` are embedded through the `owners`/`members` lookups, so a profile edit shows up a TTL late. `DELETE /v1/boards/:id` is the other: it does not broadcast, so `boardService.deleteItem` drops both board entries itself.
...
`Sentry.init` in `apps/client/src/main.tsx` is enabled only in production builds, so dev and E2E never send events. The DSN lives in `@workspace/shared/constants/sentry` because both apps need it. The browser never talks to `sentry.io`: `tunnel` posts every envelope to `POST /api/v1/diagnostics` on our own origin, and the server forwards the raw bytes upstream (`src/providers/sentry.provider.ts`), relaying `X-Sentry-Rate-Limits`/`Retry-After` so the SDK still backs off. `dataCollection.userInfo` makes Sentry infer the user's IP from whoever connects to it, which behind the tunnel is our server, so the forward sets `X-Forwarded-For` from `clientIp(request)` - the `X-Real-IP` ingress-nginx derives from Cloudflare's `CF-Connecting-IP`, falling back to `request.ip` locally. Drop that header and every event reports the cluster's egress IP. The tunnel exists because ad blockers and antivirus web filters (Kaspersky's is one) block `*.sentry.io` outright, which loses those users' errors silently. The server refuses any envelope whose header DSN is not this project's, so the route cannot be used as an open relay to other Sentry projects. Keep the forward byte-exact — replay segments are compressed binary, so decoding the body as text corrupts them. Don't add Sentry's ingest host back to the CSP `connect-src`; nothing needs it. `skipBrowserExtensionCheck` is on because an extension that injects `chrome.runtime.id` into the page otherwise makes the SDK disable itself.
...
`.github/workflows/build-k8s-images.yml` runs on `workflow_run` of `CI`, only when CI **passed** on a push to `main` (a failed run sends a Telegram notice instead and deploys nothing). It checks out `workflow_run.head_sha`, never `github.sha`, which in a `workflow_run` event is whatever `main` points to by then. It skips the build when nothing outside `infra/**` and `*.md` changed since the commit the overlay currently pins, which replaces the `paths-ignore` that `workflow_run` cannot use. Otherwise it builds both images to `ghcr.io/baoduong254/trellify-{server,client}:sha-<short>`, then a second job runs `kustomize edit set image` in `infra/trellify/overlays/prod` and pushes a `chore(deploy): ... [skip ci]` commit. The `[skip ci]` marker stops CI from running on that commit, and so stops it from triggering another build — don't remove it. `latest` is published but never deployed; the overlay always pins the immutable sha tag.
