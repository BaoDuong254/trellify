...
- REST mutations broadcast **after** `response.json()` via `broadcastBoardUpdate(request, boardId, reason)` from `src/sockets/board.broadcast.ts`. It re-fetches a board snapshot and emits `BE_BOARD_UPDATED` to the board room, but only once it has confirmed somebody is in that room — the local room first, then the cluster through the adapter. A board nobody is viewing costs one Redis round trip instead of a full snapshot, counted by `board_broadcast_skipped_total`. Keep that check in front of `getBoardSnapshot`: without it a write-heavy load spends about 30% of its server time broadcasting to nobody. Bursts against one board are then throttled over `COALESCE_WINDOW_MS` — the first send goes out immediately, the rest fold into one trailing send (`board_broadcast_coalesced_total`) that carries no `actorSocketId`, because the client drops any payload stamped with its own socket id and a burst can mix actors.
