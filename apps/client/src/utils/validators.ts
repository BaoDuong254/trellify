...
export const singleFileValidator = (file: unknown): string | null => {
  ...
  if (!fileObj.name || !fileObj.size || !fileObj.type) {
    return "File cannot be blank.";
  }
  if (fileObj.size > LIMIT_COMMON_FILE_SIZE) {
    return "Maximum file size exceeded. (10MB)";
  }
  if (!ALLOW_COMMON_FILE_TYPES.includes(fileObj.type)) {
